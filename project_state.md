...
- 切片 3:`Services/BeautyDiaryStore.swift`(2122 行,單一 God Class)已按 domain 拆為主檔(屬性/init/存檔/共用泛型方法,220 行)+ 8 個 `extension BeautyDiaryStore` 檔案(Home/Beauty/Body/Growth/Finance/AIAdvice/Resource/Profile);拆分前已先把所有跨檔案會用到的 `private func`/`private let`/`private var`/`private static` 成員改為模組內可見(記取切片 2 的教訓),並確認每個新檔案大括號配對正確;已更新 `project.yml`
...
- `Models/DiaryModels.swift` 1652 行、`Services/ResourcePipelineServices.swift` 1521 行、`Services/ResourceImportService.swift` 1063 行,均嚴重超過 300 行上限,待拆分
...
- 請在 Xcode(或觸發 CI)跑一次 build,確認切片 3 的拆分沒有破壞編譯
- 開始切片 4:拆分 `DiaryModels.swift`(1652 行)
...
| 3 | 拆分 BeautyDiaryStore.swift | 記憶/邏輯 | Services/BeautyDiaryStore.swift → 主檔 + 8 個 extension + project.yml | ✅ 已完成(待你 Xcode/CI 編譯驗證) |
